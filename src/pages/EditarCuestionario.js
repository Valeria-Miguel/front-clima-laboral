  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await fetch('http://localhost:3005/api/cuestionarios/update', {
        ...
        body: JSON.stringify({ id: cuestionario._id, nombre }),
      });
        ...
        <div className="registro-form-wrapper">
          ...
          <form className="registro-form" onSubmit={handleSubmit}>
            <div className="form-group">
              <label>Tipo:</label>
              <input type="text" value={cuestionario?.tipo || ''} disabled className="registro-input" />
            </div>
            <div className="form-group">
              <label>Cliente ID:</label>
              <input type="text" value={cuestionario?.clienteId || ''} disabled className="registro-input" />
            </div>
            <div className="form-group">
